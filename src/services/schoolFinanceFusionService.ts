...
import type { SchoolFinanceDashboardV2Data } from "@/types/schoolFinanceV2";
import type {
...
function createBudgetSnapshot(
  period: keyof typeof periodScaleMap,
): LegacyBudgetTopicSection {
  ...
  const totalExpense = Number(
    standardExpenseItems
      .reduce((sum, item) => sum + item.amount, 0)
      .toFixed(2),
  );
  const highlightItem =
    standardExpenseItems.reduce((maxItem, item) =>
      item.amount > maxItem.amount ? item : maxItem,
    ) ?? standardExpenseItems[0];
...
function recolorCategoryItems<T extends { color: string }>(items: T[]) {
...
function recolorSeries<T extends { color: string }>(items: T[]) {
  return items.map((item, index) => ({
    ...item,
...
function createThemeSnapshot<T>(section: T): T {
  const cloned = JSON.parse(JSON.stringify(section)) as any;
...
export async function fetchSchoolFinanceFusionDemo(): Promise<FinancialAnalysisFusionData> {
  const v2Data = financeDashboardV2Mock as SchoolFinanceDashboardV2Data;
