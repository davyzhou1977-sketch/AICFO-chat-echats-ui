...
export type TopicKey = "budget" | "reimbursement" | "personnel" | "guarantee";
...
export interface TopicSnapshot {
  ...
  metrics: MetricItem[];
  trendLabels: string[];
  trendSeries: Array<{ name: string; color: string; data: number[] }>;
...
export interface PeriodSnapshot {
  budget: TopicSnapshot;
  reimbursement: TopicSnapshot;
  personnel: TopicSnapshot;
  guarantee: TopicSnapshot;
}
...
function scaleSeries(series: number[], scale: number) {
  return series.map((item) => Number((item * scale).toFixed(1)));
}

function buildSnapshot(scale: number): PeriodSnapshot {
...
export const dashboardByPeriod: Record<PeriodKey, PeriodSnapshot> = {
